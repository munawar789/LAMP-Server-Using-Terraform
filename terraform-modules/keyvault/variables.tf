...
variable "location" {
  ...
  validation {
    condition     = contains(["eastus", "centralus"], lower(var.location))
    error_message = "Unsupported Azure Region specified for key-vault. Only East US and Central US Azure Regions are supported."
  }
}
variable "rg_Name" {
  type        = string
